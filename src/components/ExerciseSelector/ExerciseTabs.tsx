...
import { Activity, Shield, Dumbbell, Flame } from 'lucide-react';
        ...
        {exercises.map(({ id, icon, tag }) => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2.5 p-2.5 sm:py-3 sm:px-3.5 rounded-xl text-left transition-all duration-200 relative overflow-hidden ${
                isSelected
                  ...
                  : 'bg-transparent border border-transparent text-slate-400 hover:text-white hover:bg-slate-850'
              }`}
            >
              ...
              <div className={`p-2 rounded-lg ${isSelected ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-800/70 text-slate-400'}`}>
                {icon}
              </div>
